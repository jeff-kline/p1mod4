...
\title{A Paley--II analogue of the Scarpis--Djokovi\'c construction\\
...
one of order \(p(p+1)\), and Djokovi\'c extended the lift to all prime
...
The orders so produced are not new existence results.  Every order
...
\(q=81\) (order \(13284=4\cdot 41\cdot 81\)), is a classical Turyn
product of \(T\)-matrices of order \(41\) with Williamson-type matrices
of order \(81=9^{2}\).  We make the coverage explicit, exhibit the
witnessing constructions, identify the smallest unaudited order, and
...
Djokovi\'c \cite{Djokovic2016} replaced the prime by an arbitrary prime
...
uniform family that Djokovi\'c \cite{Djokovic2016} asked for.  It does
not, however, produce Hadamard matrices of previously unconstructible
orders, and we record why.
...
\(4\times\text{prime}\).  Second, \eqref{eq:fourodd} is itself a Turyn
factorisation: with \(T\)-matrices of order \(t\) and Williamson-type
matrices of order \(w\) one obtains a Hadamard matrix of order \(4tw\)
...
standard tables, the first order beyond that range is itself a Turyn
product, and the orders for which no elementary witness is currently
...
with \(t>2\) are the primes \(167,179,223\) (orders \(668,716,892\),
...
\newblock {\em Linear Multilinear Algebra} 65 (2017), no.\ 10;
  arXiv:1601.00635.
