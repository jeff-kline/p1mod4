...
% OT1 Computer Modern is installed here as scalable Type 1 fonts.  Define the
% crossed D used in Đoković without falling back to bitmap-only T1 fonts.
\DeclareTextCommand{\DJ}{OT1}{%
  \leavevmode\makebox[0pt][l]{\raisebox{.45ex}{\hspace{.04em}\rule{.48em}{.05em}}}D}
...
\hypersetup{
  hidelinks,
  pdftitle={A Paley-II analogue of the Scarpis-Djokovic construction for prime powers q congruent to 1 modulo 4},
  pdfauthor={Jeffery Kline},
  pdfsubject={An explicit Hadamard-matrix construction of order 2q(q+1)},
  pdfkeywords={Hadamard matrix, Paley construction, Scarpis construction, difference matrix, Butson matrix}
}
...
\date{Release candidate 0.1.0\\August 2, 2026}
...
\begin{center}
\small Living repository: \url{https://github.com/jeff-kline/p1mod4}.\\
This release candidate has no permanent archive or DOI yet.
\end{center}

\begin{abstract}
A Hadamard matrix is a square sign matrix with pairwise orthogonal rows.
For every prime power \(q\equiv1\pmod4\), we give a choice-free formula for
such a matrix of order \(2q(q+1)\).  Its finite bands are written entry by
entry from the quadratic character of \(\F_q\) and the \(2\times2\)
Paley--II blocks.  Four block-Gram identities prove orthogonality.

The existence result is due to Farouk and Wang (2020).  Their construction
lifts an input Hadamard matrix and depends on auxiliary choices; the formula
here uses neither.  At \(q=5\), it produces a matrix inequivalent to their
printed order-\(60\) example, although another choice in their procedure
produces an equivalent matrix (Remark~\ref{rem:alpha5}).

We also characterize the two ingredients that the formula needs.  On a finite
abelian group \(G\) of order \(n\), the generalized matrix is Hadamard if and
only if its sign function is of Paley type and its shift array is a
\((G,n;1)\)-difference matrix.  The sufficiency direction was known in greater
generality; the converse is apparently new within the bounded literature
corpus recorded with this release.  It forces
\(n\equiv1\pmod4\), and an example of non-prime-power order would yield a
projective plane of that order.  Finally, we express the construction as the
Turyn double of a Butson matrix over the Gaussian units and record the known
coverage of its orders.  The coverage screen identifies unaudited cases, not
open Hadamard orders.
\end{abstract}

\section{Introduction}

A Hadamard matrix of order \(N\) is an \(N\times N\) matrix with entries
in \(\{\pm1\}\) whose rows are pairwise orthogonal.  Equivalently, it
satisfies \(HH^{\mathsf T}=N\I_N\) \cite{Hadamard1893}.  This note gives an explicit formula for
one such matrix of order \(2q(q+1)\) whenever \(q\equiv1\pmod4\) is a prime
power.

The starting point is Paley's use of quadratic residues in finite fields.  Let
\(\F=\F_q\), and define the quadratic character
\(\chi\colon\F\to\{0,\pm1\}\) by \(\chi(0)=0\), with value \(1\) on nonzero
squares and \(-1\) on non-squares.  When \(q\equiv3\pmod4\), the Paley--I
construction gives a Hadamard matrix of order \(q+1\).  When
\(q\equiv1\pmod4\), Paley--II gives one of order \(2(q+1)\)
\cite{Paley1933}.

Scarpis \cite{Scarpis1898} showed that, for a prime \(p\equiv3\pmod4\), a
Hadamard matrix of order \(p+1\) can be lifted to order \(p(p+1)\).
\DJ okovi\'c \cite{Djokovic2016} extended this result from primes to prime
powers and asked for an analogue when \(q\equiv1\pmod4\).  In that case the
Paley--II seed has order \(2(q+1)\), so the corresponding target is
\[
        q\cdot 2(q+1)=2q(q+1).
\]

Farouk and Wang \cite{FaroukWang2020} already proved existence at these
orders.  Their method lifts an eligible Hadamard matrix of order \(2(q+1)\)
and depends on a chosen bijection.  The formula here is a different route to
the same existence statement.  It takes no input matrix and makes no
auxiliary choice: its finite bands have entries
\(\psi(\chi(y-x-t))\), where \(\psi\) replaces the three character values by
the Paley--II \(2\times2\) blocks.  Four block-Gram identities, computed from
the quadratic character, prove that the rows are orthogonal.  The assumption
\(q\equiv1\pmod4\) enters through \(\chi(-1)=1\), the identity that fails in
the other residue class.

The method also exposes a second result.  Section~5 replaces \(\chi\) by a
function \(\delta\colon G\to\{0,\pm1\}\) on a finite abelian group and
replaces the field shifts by an arbitrary array \(T\).  The resulting matrix
is Hadamard exactly when \(\delta\) is of Paley type and \(T\) is a
\((G,n;1)\)-difference matrix (Theorem~\ref{thm:char}).  Seberry
\cite{Seberry1980} and Nu\~nez Ponasso \cite[Thm.~4.3.3]{Ponasso2024} proved
the sufficiency direction in greater generality.  The claimed new part is the
converse: the Hadamard property forces both ingredients.

Section~6 rewrites the same construction over the Gaussian integers.  The
real matrix is the Turyn double of a Butson matrix \(\Gamma\) of half the
order (Theorem~\ref{thm:complex}).  Section~7 states the remaining open
questions.  Appendix~\ref{app:coverage} compares the resulting orders with
published tables and then applies a restricted elementary witness screen.
Orders missed by that screen are recorded as unaudited, not as open Hadamard
orders.
...
composing with the doubling of Section~6 recovers the ``if'' half of
...
no other input.  A bounded search also covered Jungnickel's difference-matrix
papers, de Launey's surveys and developed-matrix work, and de Launey and
Flannery's algebraic-design framework
\cite{Jungnickel1979,Jungnickel1980,deLauney1986,deLauney1992,
deLauneyFlannery2011,ColbournDinitz2007}; it did not locate this necessity
statement.  We therefore describe the converse only as \emph{apparently new
within that corpus}, not as a claim of global priority.  The exact corpus,
queries, access limits, and confidence judgment are recorded in
\texttt{audit/PRIOR\_WORK.md}.  This is the direction
Corollaries~\ref{cor:mod4} and \ref{cor:plane} rest on.

\begin{proof}
Let \(B\) be as in \eqref{eq:blocksum} and put
\(\mu_{rs}(u)=\#\{a\in G:\,T(r,a)-T(s,a)=u\}\).  Rerunning the block
computations of Section~3 with \(\chi\) replaced by \(\delta\) and \(ar\) by
...
closed-form construction of the present shape.  The layout of Section~3 produces
...
The blocks \(P\), \(Z\), \(R\) of Section~2 are not three unrelated objects: they
...
Nothing in Sections~1--4 depends on this section.
...
\]
the third identity being the relation \(ZR=-P\) of Section~2 rewritten together
with \(PR=-RP\).  Define
...
\cite{Cohn1965} and Turyn \cite{Turyn1970}; see also the general treatment
of Butson-class morphisms in \cite{EganOCathain2019}.  We call
\(\varphi(\Gamma)\) the \emph{Turyn double} of \(\Gamma\).
...
Lemma~\ref{lem:double} converts the four block-Gram identities of Section~3 into
...
output.  Over \(\Z[i]\), then, the construction of Section~3 is the instance of
theirs in which the seed, the deleted row and the bijection \(\alpha\) are
all determined by \(q\); the distinction drawn in Section~4 against
...
however, strictly closer than the one recorded in Section~4.  There we note in
...
the argument of Section~3 --- the residue class is used in one place only, the
...
inequivalence recorded in Section~4 is to the order-\(60\) matrix printed in
...
levels: the audited tables (Section~\ref{app:audit}), the first unaudited
order (Section~\ref{app:81}), a second worked example lying beyond the reach
of the elementary test (Section~\ref{app:109}), and that elementary-witness
test itself, which isolates the candidate orders
(Section~\ref{app:candidates}).
...
73 & 10804 & \(37\cdot73\)      & tables; no restricted witness (see Section~\ref{app:candidates})\\
\end{tabular}
\end{center}
...
Section~\ref{app:candidates} supplies no witness is
\[
...
independently, \cite[Sec.~2]{Djokovic2011} records that \(T\)-sequences of
...
\(W\)-known in the sense of Section~\ref{app:candidates}, and Seberry and
...
Section~\ref{app:candidates} misses \(q=109\) because its \(W\)-known class
...
of the test as stated; as with \(q=73\) in Section~\ref{app:audit}, the
...
\eqref{eq:fourodd} and Turyn's array, whose four-circulant antecedent is due
to Williamson \cite{Williamson1944}.  We call an odd order \(t\)
...
The audited range of Section~\ref{app:audit} is a table lookup, and the
\(q=81\) witness of Section~\ref{app:81} cites its two ingredients
individually.  Both are independent of this correction.  Nothing in
Sections~1--4, and in particular neither Theorem~\ref{thm:main} nor its proof,
...
Williamson-type matrices of order \(109\), and Section~\ref{app:109} supplies
...
entry of the table already illustrates this: Section~\ref{app:109} covers
\(q=109\) by a mechanism the test does not admit.  Widening the
\(T\)-known class would remove further entries --- \cite[Sec.~2]{Djokovic2011}
...
with no restricted elementary witness (Section~\ref{app:candidates}),
...
\bibitem{deLauney1986}
W.~de~Launey,
\newblock A survey of generalized Hadamard matrices and difference matrices
  \(D(k,\lambda;G)\) with large \(k\),
\newblock {\em Utilitas Math.} 30 (1986), 5--29.

\bibitem{deLauney1992}
W.~de~Launey,
\newblock Generalised Hadamard matrices which are developed modulo a group,
\newblock {\em Discrete Math.} 104 (1992), no.~1, 49--65.
\newblock \url{https://doi.org/10.1016/0012-365X(92)90624-O}.

\bibitem{deLauneyFlannery2011}
W.~de~Launey and D.~L.~Flannery,
\newblock {\em Algebraic Design Theory},
\newblock Mathematical Surveys and Monographs 175, American Mathematical
  Society, Providence, RI, 2011.
\newblock \url{https://doi.org/10.1090/surv/175}.
