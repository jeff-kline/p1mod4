...
who constructed a Hadamard matrix of order \(2q(q+1)\) from the
order-\(2(q+1)\) Paley--II seed for every prime power \(q\equiv 1\pmod 4\).
We re-derive that family in an explicit, self-contained form built
directly from the quadratic character on \(\F_q\) and the \(2\times 2\)
Paley--II replacement blocks; this is an alternative presentation of the
Farouk--Wang construction, not a new existence result.  Indeed every order
...
order-\(2(q+1)\) Paley--II seed; Theorem~\ref{thm:main} is therefore an
alternative, self-contained presentation rather than a new existence
result.  It does not resolve previously open Hadamard orders, and we
record the evidence for that framing.
