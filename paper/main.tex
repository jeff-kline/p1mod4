...
\begin{abstract}
For a prime power \(q\equiv 1\pmod 4\) we write down a Hadamard matrix
of order \(2q(q+1)\) in closed form: the finite bands are given
entrywise by the quadratic character on \(\F_q\) composed with the
\(2\times2\) Paley--II replacement blocks, with no input matrix and no
choices; orthogonality follows from four block-Gram identities in
character sums.  The existence statement is not new: it is Farouk and
Wang's (2020), whose lift from order \(2(q+1)\) takes choices that can
change the output.  The contribution is the method: formula rather than
procedure.  At \(q=5\) our matrix is not Hadamard-equivalent to the
order-\(60\) example they print, though it need not differ from every
output of their procedure (Remark~\ref{rem:alpha5}).

We then ask what the construction needs.  On a finite abelian group
\(G\) of order \(n\), replace the character by
\(\delta\colon G\to\{0,\pm1\}\) with \(\delta(0)=0\) and the shift by an
arbitrary array \(T\); the matrix is Hadamard \emph{if and only if}
\(\delta\) is of Paley type and \(T\) is a \((G,n;1)\)-difference
matrix.  Sufficiency is known in greater generality (Seberry;
Nu\~nez Ponasso); new is the converse, that both are forced.  Paley
type forces \(n\equiv1\pmod4\), and the difference-matrix condition
yields \(n-1\) mutually orthogonal Latin squares, so an instance on a
group of non-prime-power order would give a projective plane of
non-prime-power order.

The Paley--II blocks are the Gaussian units in the real \(2\times2\)
representation, and our matrix is the Turyn double of a Butson matrix
\(\Gamma\in BH\bigl(q(q+1),4\bigr)\) that is equivalent to an output of
the complex Scarpis lift of Sargent, Lee and Rushall (2024).  An
appendix makes coverage explicit: orders with odd part at most
\(3000\) are in the standard tables, the first unaudited order (\(q=81\))
follows from a classical Turyn product, and for \(q\le3000\) we
tabulate the orders a restricted elementary screen fails to witness ---
\emph{unaudited} rather than open.
\end{abstract}
...
\(q\equiv1\pmod4\) is due to Farouk and Wang \cite{FaroukWang2020}; that
existence statement is not new here, and the contribution of this note is
the method of construction and proof.  Farouk and Wang obtain the matrix
as the output of a lifting procedure that varies with an input Hadamard
matrix of order \(2(q+1)\) and a bijection, verified row pair by row pair
from the input matrix's Hadamard property.  Here there is no input matrix
and no bijection: the finite bands are written entrywise in the closed
form \(\psi(\chi(y-x-t))\), and the Hadamard property follows from four
block-Gram identities computed directly from the quadratic character.

\S5 asks what this construction requires: replacing \(\chi\) by a
function \(\delta\colon G\to\{0,\pm1\}\) with \(\delta(0)=0\) and the
shift by an arbitrary array \(T\), the resulting matrix is Hadamard
\emph{if and only if} \(\delta\) is of Paley type and \(T\) is a
\((G,n;1)\)-difference matrix (Theorem~\ref{thm:char}); sufficiency is
prior art, due to Seberry \cite{Seberry1980} and to Nu\~nez Ponasso
\cite[Thm.~4.3.3]{Ponasso2024}, and the converse is new here.  \S6 reads
the same matrix over the Gaussian integers: it factors through a Butson
matrix \(\Gamma\) of half the order whose Turyn double it is
(Theorem~\ref{thm:complex}).  \S7 collects the open problems these
readings raise.  A further contribution is the coverage analysis of
Appendix~\ref{app:coverage}, which has no counterpart in
\cite{FaroukWang2020}: it locates the initial range
